...
`.specsmd/aidlc-turbo/templates/construction/bolt-types/{bolt_type}.md`
...
Build a unit through the DDD stages (lean — default), or execute a planned bolt (full —
legacy), producing artifacts with human validation at checkpoints defined by the stage
playbook. Honor every caveat in the decisions-and-gates ledger.

---

## Mode (lean default; full is a legacy escape hatch)

- **lean (default):** driven by a **unit** (`--unit`). There is no `bolt.md`; the stories
  in the unit plus the decisions-and-gates ledger are the scope. Use Step 1a + 3a.
- **full (legacy):** driven by a planned **bolt** (`--bolt-id`). Use Step 1b + 3b.

Resolve to lean unless `--bolt-id` is given.

---

## Input

- **Required (lean)**: `--unit` - the unit to build; **or (full)**: `--bolt-id` - the bolt instance ID
- **Required**: `.specsmd/aidlc-turbo/memory-bank.yaml` - artifact schema
- **Required (lean)**: `{intent}/decisions-and-gates.md` - caveats, decisions, release gates
...
### 1a. Load Unit Context (lean — default)

- Read the unit's stories: `{intent}/units/{unit}/stories/*.md` — these (their acceptance
  criteria) are the scope. Read the lean unit stub `{intent}/units/{unit}/unit-brief.md`
  for the scope/ownership hint.
- **Read `{intent}/decisions-and-gates.md` in full.** Note every caveat whose build rule
  governs these stories; you will honor them during implementation and must NOT silently
  resolve an `OPEN` one. Note the release gates that apply to this unit.
- Determine the stage playbook (default `ddd-construction-bolt`) and the current stage from
  the unit's construction-log (or start at the first stage). Then continue at Step 2.

### 1b. Load Bolt Context (full — legacy)
...
### 2. Load Stage Playbook Definition (CRITICAL)

**Lean:** use the default playbook `ddd-construction-bolt` (from Step 1a).
**Full:** use the `bolt_type` extracted in Step 1b.

**Location**: `.specsmd/aidlc-turbo/templates/construction/bolt-types/{playbook}.md`
...
### 3. Load Unit Context (full — legacy; lean already did this in Step 1a)

**Full mode only.** After extracting the unit from the bolt file, load its brief for
context. (In lean mode you already loaded the lean unit stub + stories + decisions-and-gates
in Step 1a — skip this step.)
...
Load context as defined in `.specsmd/aidlc-turbo/context-config.yaml` for the `construction` agent:
...
│  → Re-read Step 10 NOW                                     │
│  → You MUST run: node .specsmd/aidlc-turbo/scripts/bolt-complete.cjs │
...
```bash
node .specsmd/aidlc-turbo/scripts/bolt-complete.cjs {bolt-id}
```
...
`.specsmd/aidlc-turbo/templates/construction/construction-log-template.md`
