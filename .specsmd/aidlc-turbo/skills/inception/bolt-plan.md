...
bolts, create no `.specs-aidlc-turbo/bolts/` directories, write no `bolt.md` files. Return
...
- **Required**: `.specsmd/aidlc-turbo/templates/standards/catalog.yaml` - project type definitions
...
3. **Bolt type templates** are located at:
   `.specsmd/aidlc-turbo/templates/construction/bolt-types/{bolt-type}.md`
...
1. **Read Path**: Check `schema.bolts` from `.specsmd/aidlc-turbo/memory-bank.yaml`
   *(Default: `.specs-aidlc-turbo/bolts/{bolt-id}/`)*
...
2. **Determine Bolt ID**:
   ...
   **⚠️ CRITICAL**: The `{BBB}` prefix is a **GLOBAL** sequence across ALL bolts in `.specs-aidlc-turbo/bolts/`, NOT per-unit.

3. **Create Directory + File Per Bolt**:
   For EACH bolt in the plan:
   - Create directory: `.specs-aidlc-turbo/bolts/{BBB}-{unit-name}/`
   - Create file inside: `.specs-aidlc-turbo/bolts/{BBB}-{unit-name}/bolt.md`
   ...
   **Naming Convention** (from `memory-bank.yaml`):
   ...
   - The number is global across ALL bolts in `.specs-aidlc-turbo/bolts/` (not per-unit)
   ...
   **Stage artifacts will be added to same directory during construction:**
   ...
   ```text
   .specs-aidlc-turbo/bolts/001-auth-service/
...
```text
.specs-aidlc-turbo/bolts/{BBB}-{unit-name}/bolt.md  ← CREATE THIS DIRECTORY AND FILE
```
...
### Directories Created
✅ `.specs-aidlc-turbo/bolts/001-auth-service/bolt.md`
✅ `.specs-aidlc-turbo/bolts/002-auth-service/bolt.md`
✅ `.specs-aidlc-turbo/bolts/003-auth-service/bolt.md`
