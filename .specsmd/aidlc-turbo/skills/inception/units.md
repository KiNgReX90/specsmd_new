...
- **Required**: `.specs-aidlc-turbo/project.yaml` - project configuration (for project_type)
...
1. **Read project type** from `.specs-aidlc-turbo/project.yaml`:
...
1. **Read Path**: Check `schema.units` from `.specsmd/aidlc-turbo/memory-bank.yaml`
   *(Default: `.specs-aidlc-turbo/intents/{intent-name}/units.md`)*
...
4. **Create Unit Brief** (CRITICAL):
   ...
   **Lean mode → use "Step 7 — Lean variant" below instead of this verbose brief.**
   ...
   For each unit (full mode), create `{UUU}-{unit-name}/unit-brief.md` using `.specsmd/aidlc-turbo/templates/inception/unit-brief-template.md`
