...
1. **Read Path**: Check `schema.stories` from `.specsmd/aidlc-turbo/memory-bank.yaml`
   *(Default: `.specs-aidlc-turbo/intents/{intent-name}/units/{unit-name}/stories/`)*
...
3. **Create Story Files** (IMPORTANT - ONE FILE PER STORY):
   ...
   **Read naming convention from `.specsmd/aidlc-turbo/memory-bank.yaml`**
   ...
   **DO NOT** create a single `stories.md` file with all stories
   ...
   Use template: `.specsmd/aidlc-turbo/templates/inception/story-template.md`
   ...
   **Global uniqueness** comes from the full path:
   `.specs-aidlc-turbo/intents/001-user-authentication/units/auth-service/stories/001-user-signup.md`
...
1. **Read Configuration**: Check `story-index` settings in `.specsmd/aidlc-turbo/memory-bank.yaml`
...
3. **Based on mode**:
   ...
   **Option 1: single-file** (default)
   - Path: `.specs-aidlc-turbo/story-index.md`
   - Add all stories to the central index with full paths

   **Option 2: per-intent**
   - Path: `.specs-aidlc-turbo/intents/{intent-name}/story-index.md`
