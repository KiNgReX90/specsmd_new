...
- **Optional**: `.specs-aidlc-turbo/project.yaml` - project type for context-aware suggestions
...
1. Read `.specs-aidlc-turbo/project.yaml` to get `project_type`
2. List `.specs-aidlc-turbo/intents/` directory to see existing intents
...
1. **Read existing intent names** from `.specs-aidlc-turbo/intents/` folder names
...
1. **Read Schema**: Check `.specsmd/aidlc-turbo/memory-bank.yaml` for `intents` path
   *(Default: `.specs-aidlc-turbo/intents/{intent-name}/`)*
...
3. **Create `requirements.md`**:
   - Use template: `.specsmd/aidlc-turbo/templates/inception/requirements-template.md`
