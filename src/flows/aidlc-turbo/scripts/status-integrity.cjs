 ...
 *
 * Usage:
 *   node .specsmd/aidlc-turbo/scripts/status-integrity.cjs
 *   node .specsmd/aidlc-turbo/scripts/status-integrity.cjs --fix
 *
...
const MEMORY_BANK_DIR = '.specs-aidlc-turbo';
...
|----------|------------|------------|--------|
${fixedItems.map(item => {
    const shortPath = item.path.replace(/^\.specs-aidlc-turbo\//, '');
...
async function statusIntegrity(fix = false) {
    ...
    if (inconsistencies.length === 0) {
        ...
        for (const issue of inconsistencies) {
            const shortPath = issue.path.replace(/^\.specs-aidlc-turbo\//, '').replace(/^.*\/intents\//, '');
        ...
        if (fix) {
            ...
            for (const issue of inconsistencies) {
                ...
                if (fixed) {
                    console.log(`  ${colors.green}✓${colors.reset} Fixed: ${issue.path.replace(/^\.specs-aidlc-turbo\//, '')}`);
                    fixedItems.push(issue);
                } else {
                    console.log(`  ${colors.red}✗${colors.reset} Failed: ${issue.path.replace(/^\.specs-aidlc-turbo\//, '')}`);
                }
            }
