...
4. A one-item request is a one-item intent, graded `low` when its change spec is fully written, unless it fits the fix-now box: one piece of work in about five source files or fewer that no pending or in-progress item owns, its test lands with it, no look nobody has decided, no encoding of an external rule the project tracks. A one-item intent's state entry comment carries a `BOX.` line naming the exit it took, which `state-transition.cjs check` requires. Such an item is returned as a `fix-now` block for the parent conversation to fix in the same turn and never enters the ledger. Never write `.specs-inferno/quick-fixes.md` or any other parking file; `state-transition.cjs check` reports it as drift. Couple work that changes one surface; never bundle unrelated small fixes into a catch-all intent.
