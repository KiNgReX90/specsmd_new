...
<config>
  Optional per-project file `.specs-fire/config.yaml` adapts this flow to the host project so the flow files themselves stay project- and host-neutral (annotated template: `.specsmd/fire/agents/team/config.example.yaml`; `/specsmd-fire-team-config` creates it interactively). Read it once at activation. Every key is optional; a missing key falls back as stated:
