...
<flow>
  ...
  <step n="3c" title="One item is a one-item intent (nothing is parked)" critical="true">
    ...
    <action>The handoff readout leads with the item count of every captured intent.</action>
  </step>
...
<success_criteria>
  ...
  <criterion>Every captured request landed in state.yaml as an intent, a one-item request as a one-item intent; no `quick-fixes.md` or other parking file written; unrelated small items never bundled into a catch-all intent</criterion>
