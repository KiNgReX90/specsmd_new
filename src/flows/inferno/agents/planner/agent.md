...
**The fix-now box is not the ledger's.** Size decides first. One piece of work in about five source files or fewer, whose test lands with it, that no pending or in-progress item owns, that decides no look nobody has decided and encodes no external rule the project tracks, is not an intent, whatever its lane. A value the design system already declares and a file misses is a defect, whatever it does to the pixels. Measure the size before routing: name the mechanism at file and line and count the files the fix reaches. Return it as a `fix-now:` block: the change, the files, the test or measurement that proves it, what you measured, and the word `hard` when the fix has more than one defensible shape or a cause your measurement did not settle, so the launching session asks the oracle before its builder builds it; a block without `hard` goes to the builder tier as it stands, the box as its ownership and the proving test as its test. Write no brief, no item and no ledger entry for it. Work that leaves the box is an intent, a one-item intent when that is all it is, and a one-item intent's ledger comment carries a `BOX.` line naming the exit with its measurement.
...
5. **Success criteria are measured, before and after.** Each names its instrument (a test file, a tester case id, a command) and the value it must show when the intent is done. A criterion a person would judge by looking is a case for the tester.
...
- No standalone verify item by default. A cheap mechanical invariant rides as `finalize_check:` on the item that owns the change. Emit a `kind: test` item only when verification needs reasoning a one-line command cannot express, such as a case list the tester walks on the binary.
...
- The entry comment is the changelog, three to ten lines: the source, the measured ground truth, why the chain is shaped as it is, the cross-intent decisions, the tester case ids, and the look change when there is one, and for a one-item intent a `BOX.` line naming the exit it took out of the fix-now box with its measurement, which `check` requires.
