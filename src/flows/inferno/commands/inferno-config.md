...
6. Write `.specs-inferno/config.yaml`, preserving any existing keys you did not touch. Keep the file minimal: only keys that differ from the documented defaults. Every key stays optional; `autonomy.level` changes nothing in the planner either way, and an omitted `delivery.mode` behaves as `auto-close`. A `models.writer` key from an older config is obsolete: the planner writes its own work items, so drop it.
