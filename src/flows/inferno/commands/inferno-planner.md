...
- **One work item is a one-item intent. Nothing is parked.** Every request lands in state.yaml as an intent (intent-capture step 3c), a one-item request as a one-item intent, because the ledger is the only queue any session builds from. Never write `.specs-inferno/quick-fixes.md` or any other parking file: `state-transition.cjs check` reports that file as drift the moment it exists, and parked work is never built (2026-08-30: three quick fixes captured on 2026-08-27 beside five intents were untouched three days later while the intents shipped). A dependency on an open intent is `depends_on_intents`, a shared file is the collision rule of work-item-decompose step 6b, a user look at the running result is a USER-VERIFIES line in the entry comment, and a fully written change spec is graded `low` so the orchestrator runs it on the cheap builder tier. An open design question still goes to the oracle first (`specsmd-inferno-oracle`, canonical body `.specsmd/inferno/agents/oracle/agent.md`): you cannot spawn one, so return the question as an `oracle:` block to whoever dispatched you and write nothing until the decision comes back. Grouping: COUPLING groups (same surface or files = one intent); never bundle unrelated smalls into a catch-all intent.
...
Your final message is your only output the user sees: lead with the gate readout per request (item count per intent), then what you captured, then the load-bearing ground-truth findings, then the dependency decisions. Compact. Paths and facts, no file bodies.
