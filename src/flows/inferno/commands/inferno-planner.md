...
- **Small work may not deserve the flow.** When the whole intent is a short serial chain of small, already-understood changes (e.g. minor UI deltas from a finished design study), name the tradeoff in your handoff: building via `/specsmd-inferno` pays one cold builder dispatch per item or batch, while implementing directly from the captured specs in the capturing session is materially cheaper. Offer both routes; the user picks. The specs + state.yaml entry keep their value either way.
