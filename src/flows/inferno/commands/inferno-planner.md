...
- **Small work may not deserve an intent at all — gate at capture, not at handoff.** Before writing any brief, apply the intent-worthiness gate (intent-capture step 3c): a request that would decompose to a SINGLE low-complexity work item, is independent of every open intent (no shared files, no depends_on), carries no elevated risk (no architecture/security/data implications, no live-app or user-sign-off verification), and is already fully understood does NOT become an intent. Park it in `.specs-inferno/quick-fixes.md` instead (grounded spec: what + exact files + acceptance + verify; status `open`) for direct in-session implementation. Grouping when a capture brings several items: COUPLING groups (same surface/files = one intent), SIZE gates; never bundle unrelated smalls into a catch-all intent — that widens ownership and blocks parallel builds. A dependency on an open intent always makes the work intent-worthy (the bookkeeping is what the intent buys), and the user explicitly asking for an intent always overrides. For work that passes the gate but is still a short serial chain of small, already-understood items, keep naming the cost tradeoff in the handoff: `/specsmd-inferno` pays one cold builder dispatch per item or batch; implementing directly from the captured specs is materially cheaper. Offer both routes; the user picks.
